 ...
 * Props JSON Schema: {"type":"object","properties":{"externalUrl":{"type":"string"}}}
 * State Schema: {"type":"object","properties":{"websiteUrl":{"type":"string"},"loading":{"type":"boolean"},"auditResult":{"type":"object"},"error":{"type":"string"},"activeView":{"type":"string"}}}
 * Audit Result Schema: {"type":"object","properties":{"url":{"type":"string"},"accessibility_score":{"type":"number"},"overall_score":{"type":"number"},"lawsuit_risk":{"type":"string","enum":["low","medium","high"]},"wcag_level":{"type":"string","enum":["AA","AAA"]}}}
 */
...
const buildProjectedResult = (result) => {
  const acc = Math.min(
    100,
    Math.round(
      result.accessibility_score + (100 - result.accessibility_score) * 0.88,
    ),
  );
  const seo = Math.min(
    100,
    Math.round(result.seo_score + (100 - result.seo_score) * 0.82),
  );
  const perf = Math.min(
    100,
    Math.round(
      result.performance_score + (100 - result.performance_score) * 0.75,
    ),
  );
  const sec = Math.min(
    100,
    Math.round(result.security_score + (100 - result.security_score) * 1.0),
  );
  const res = Math.min(
    100,
    Math.round(
      (result.resources_score || 50) +
        (100 - (result.resources_score || 50)) * 0.8,
    ),
  );
  const img = Math.min(
    100,
    Math.round(
      (result.images_score || 50) + (100 - (result.images_score || 50)) * 0.85,
    ),
  );
  const net = Math.min(
    100,
    Math.round(
      (result.network_caching_score || 50) +
        (100 - (result.network_caching_score || 50)) * 0.9,
    ),
  );
  const cq = Math.min(
    100,
    Math.round(
      (result.code_quality_score || 50) +
        (100 - (result.code_quality_score || 50)) * 0.78,
    ),
  );
...
const ScoreBadge = ({ score, label, delta }) => {
  ...
  return (
    <div className="flex flex-col items-center">
      <div
        className={`w-16 h-16 rounded-full flex items-center justify-center ring-4 ${colorClasses}`}
      >
    ...
    <div>
      {vitals.source && (
        <div className="mb-3 flex items-center gap-2">
          <span
            className={`text-xs px-2 py-0.5 rounded-full font-medium ${vitals.source === "pagespeed" ? "bg-emerald-100 text-emerald-700" : "bg-yellow-100 text-yellow-700"}`}
          >
            {vitals.source === "pagespeed"
      ...
      <div className="grid md:grid-cols-3 gap-4">
        ...
        <MetricCard
          label="Speed Index"
          value={vitals.speed_index}
          unit="s"
          status={vitals.speed_index_status}
          target="< 3.4s"
        />
        <MetricCard
        ...
        <MetricCard
          ...
          value={vitals.inp}
          unit="ms"
          status={vitals.inp_status}
          target="< 200ms"
        />
        {vitals.fully_loaded_time != null && (
          <MetricCard
            label="Fully Loaded Time"
            value={vitals.fully_loaded_time}
            unit="s"
            status={
              vitals.fully_loaded_time < 5
                ? "good"
                : vitals.fully_loaded_time < 8
                  ? "needs-improvement"
                  : "poor"
            }
            target="< 5s"
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-4">
        <div
          className={`w-12 h-12 rounded-full flex items-center justify-center ${mobile.is_mobile_friendly ? "bg-emerald-100" : "bg-red-100"}`}
        >
          <Smartphone
            className={`w-6 h-6 ${mobile.is_mobile_friendly ? "text-emerald-600" : "text-red-600"}`}
          />
        </div>
        <div>
          <p
            className={`font-semibold ${mobile.is_mobile_friendly ? "text-emerald-700" : "text-red-700"}`}
          >
            {mobile.is_mobile_friendly
              ? "Mobile Friendly"
              : "Not Mobile Friendly"}
          </p>
        </div>
      ...
      <div className="space-y-2">
        <CheckItem
          passed={mobile.viewport_configured}
          label="Viewport meta tag configured"
        />
        <CheckItem
          passed={mobile.text_readable}
          label="Text readable without zooming"
        />
        <CheckItem
          passed={mobile.tap_targets_sized}
          label="Tap targets properly sized"
        />
        <CheckItem
          passed={!mobile.content_wider_than_screen}
          label="Content fits viewport width"
        />
      </div>
    </div>
  );
...
const SecuritySection = ({ security }) => {
  const CheckItem = ({ passed, label, desc }) => (
    <div className="flex items-start gap-3 p-3 bg-gray-50 rounded-lg">
      ...
      <div>
        <p
          className={`font-medium ${passed ? "text-gray-700" : "text-red-700"}`}
        >
          {label}
        </p>
  ...
  return (
    <div className="space-y-3">
      <CheckItem
        ...
        label="HTTPS Enabled"
        desc="Secure connection with SSL/TLS"
      />
      <CheckItem
      ...
      <CheckItem
        ...
        desc="CSP headers configured"
      />
      <CheckItem
        passed={!security.mixed_content}
        label="No Mixed Content"
        desc="All resources over HTTPS"
      />
    </div>
  );
};
...
const StructuredDataSection = ({ data }) => (
  <div>
    <div className="flex items-center gap-3 mb-4">
      <div
        className={`w-12 h-12 rounded-full flex items-center justify-center ${data.has_schema ? "bg-emerald-100" : "bg-yellow-100"}`}
      >
        <Globe
          className={`w-6 h-6 ${data.has_schema ? "text-emerald-600" : "text-yellow-600"}`}
        />
      </div>
      <div>
        <p
          className={`font-semibold ${data.has_schema ? "text-emerald-700" : "text-yellow-700"}`}
        >
...
const IssueDisplay = ({ issue, idx }) => {
  ...
  if (issue.impact === "critical") impactClass = "bg-red-200 text-red-700";
    ...
    impactClass = "bg-orange-200 text-orange-700";
  else if (issue.impact === "moderate")
    impactClass = "bg-yellow-200 text-yellow-700";
    ...
    <div>
      {summary && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
          ...
          <div className="bg-gray-50 rounded-lg p-3 text-center">
            <p className="text-xl font-bold text-gray-800">
              {summary.total_stylesheets ?? "—"}
            </p>
            <p className="text-xs text-gray-500">Stylesheets</p>
          </div>
          <div className="bg-gray-50 rounded-lg p-3 text-center">
            <p
              className={`text-xl font-bold ${(summary.dom_element_count || 0) > 1500 ? "text-yellow-600" : "text-gray-800"}`}
            >
          ...
          <div className="bg-gray-50 rounded-lg p-3 text-center">
            <p
              className={`text-xl font-bold ${(summary.render_blocking_count || 0) > 0 ? "text-red-600" : "text-emerald-600"}`}
            >
    ...
    <div>
      {summary && (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-4">
          ...
          <div className="bg-gray-50 rounded-lg p-3 text-center">
            <p
              className={`text-xl font-bold ${(summary.without_alt || 0) > 0 ? "text-red-600" : "text-emerald-600"}`}
            >
          ...
          <div className="bg-gray-50 rounded-lg p-3 text-center">
            <p
              className={`text-xl font-bold ${(summary.without_dimensions || 0) > 0 ? "text-yellow-600" : "text-emerald-600"}`}
            >
          ...
          <div className="bg-gray-50 rounded-lg p-3 text-center">
            <p
              className={`text-xl font-bold ${(summary.not_lazy_loaded || 0) > 0 ? "text-yellow-600" : "text-emerald-600"}`}
            >
          ...
          <div className="bg-gray-50 rounded-lg p-3 text-center">
            <p
              className={`text-xl font-bold ${(summary.legacy_format || 0) > 0 ? "text-yellow-600" : "text-emerald-600"}`}
            >
  ...
  return (
    <div className="space-y-3">
      ...
      <CheckItem
        ...
        label={
          network.cdn_detected
            ? `CDN: ${network.cdn_name || "Detected"}`
            : "No CDN detected"
        }
      ...
      <CheckItem
        ...
        label={
          network.cache_control
            ? "Cache-Control configured"
            : "Cache-Control missing"
        }
        desc={
          network.cache_control ||
          "No caching headers set for the HTML document"
        }
      />
      <CheckItem
      ...
      {(network.preconnects || []).length > 0 && (
        <div className="p-3 bg-emerald-50 rounded-lg">
          <p className="font-medium text-emerald-700 text-sm mb-1">
    ...
    <div>
      {summary && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
          <div className="bg-gray-50 rounded-lg p-3 text-center">
            <p
              className={`text-xl font-bold ${summary.has_font_display ? "text-emerald-600" : "text-yellow-600"}`}
            >
          ...
          <div className="bg-gray-50 rounded-lg p-3 text-center">
            <p className="text-xl font-bold text-gray-800">
              {summary.font_preloads ?? 0}
            </p>
            <p className="text-xs text-gray-500">Font Preloads</p>
          </div>
          <div className="bg-gray-50 rounded-lg p-3 text-center">
            <p
              className={`text-xl font-bold ${summary.has_document_write ? "text-red-600" : "text-emerald-600"}`}
            >
          ...
          <div className="bg-gray-50 rounded-lg p-3 text-center">
            <p
              className={`text-xl font-bold ${summary.has_source_maps ? "text-emerald-600" : "text-gray-600"}`}
            >
...
const FixesAppliedSection = ({ fixes }) => {
  ...
  return (
    <div className="space-y-4">
        ...
        >
          ...
          <div className="grid md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-200">
            <div className="p-3">
              ...
              <pre className="text-xs bg-red-50 p-2 rounded overflow-x-auto font-mono whitespace-pre-wrap">
                {fix.before}
              </pre>
            </div>
            <div className="p-3 relative">
              ...
              <pre className="text-xs bg-emerald-50 p-2 rounded overflow-x-auto font-mono whitespace-pre-wrap">
                {fix.after}
              </pre>
            </div>
          </div>
      ...
      {fixes.length > 12 && (
        <p className="text-sm text-gray-500 text-center">
          + {fixes.length - 12} more fixes available
        </p>
      )}
    </div>
  );
...
const FullAuditResults = ({ result, originalResult, view, onToggle }) => {
  ...
  const overallDelta = isWebenablix
    ? result.overall_score - originalResult.overall_score
    : 0;
  const accDelta = isWebenablix
    ? result.accessibility_score - originalResult.accessibility_score
    : 0;
  const seoDelta = isWebenablix
    ? result.seo_score - originalResult.seo_score
    : 0;
  const perfDelta = isWebenablix
    ? result.performance_score - originalResult.performance_score
    : 0;
  const secDelta = isWebenablix
    ? result.security_score - originalResult.security_score
    : 0;
  const resDelta = isWebenablix
    ? (result.resources_score || 0) - (originalResult.resources_score || 0)
    : 0;
  const imgDelta = isWebenablix
    ? (result.images_score || 0) - (originalResult.images_score || 0)
    : 0;
  const netDelta = isWebenablix
    ? (result.network_caching_score || 0) -
      (originalResult.network_caching_score || 0)
    : 0;
  const cqDelta = isWebenablix
    ? (result.code_quality_score || 0) -
      (originalResult.code_quality_score || 0)
    : 0;
    ...
    <div className="mt-12 max-w-5xl mx-auto">
      ...
      {isWebenablix && (
        <div className="flex items-center gap-2 justify-center mb-4 text-emerald-300 text-sm">
          ...
          <span>
            Projected scores after Webenablix remediates all detected issues
          </span>
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl shadow-2xl overflow-hidden">
        {/* Header */}
        <div
          className={`p-6 text-white ${isWebenablix ? "bg-gradient-to-r from-emerald-600 to-teal-500" : "bg-gradient-to-r from-[#2563EB] to-[#3B82F6]"}`}
        >
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <p className="text-sm opacity-80">
                {isWebenablix
                  ? "Projected Report — With Webenablix"
            ...
            <div className="flex justify-center mt-2">
              <span
                className={`text-2xl font-bold px-3 py-1 rounded-lg ${
                  result.grade_report.overall.grade === "A"
                    ? "bg-emerald-100 text-emerald-700"
                    : result.grade_report.overall.grade === "B"
                      ? "bg-blue-100 text-blue-700"
                      : result.grade_report.overall.grade === "C"
                        ? "bg-yellow-100 text-yellow-700"
                        : result.grade_report.overall.grade === "D"
                          ? "bg-orange-100 text-orange-700"
                          : "bg-red-100 text-red-700"
                }`}
              >
                Grade: {result.grade_report.overall.grade}
              </span>
            </div>
          )}
        </div>
        ...
        <div className="p-6 border-b">
          <div className="grid grid-cols-4 md:grid-cols-8 gap-4 justify-items-center">
            ...
            />
            <ScoreBadge
              score={result.resources_score || 0}
              label="Resources"
              delta={resDelta}
            />
            <ScoreBadge
              score={result.images_score || 0}
              label="Images"
              delta={imgDelta}
            />
            <ScoreBadge
              score={result.network_caching_score || 0}
              label="Network"
              delta={netDelta}
            />
            <ScoreBadge
            ...
            />
            <ScoreBadge
              score={result.code_quality_score || 0}
              label="Code Quality"
              delta={cqDelta}
            />
          </div>
            ...
            {isWebenablix ? (
              <>
                ...
                <p className="text-xs text-gray-400 line-through">
                  {originalResult.critical_issues} critical
                </p>
              </>
            ) : (
          ...
          <div className="p-4 text-center border-r">
            {isWebenablix ? (
              <>
                ...
                <p className="text-xs text-gray-400 line-through">
                  {originalResult.warnings} warnings
                </p>
              </>
            ) : (
          ...
          <div className="p-4 text-center">
            {isWebenablix ? (
              <>
                ...
                <p className="text-xs text-gray-400 line-through">
                  {originalResult.total_issues} total
                </p>
              </>
            ) : (
        ...
        {recs.length > 0 && (
          <div
            className={`p-6 border-b ${isWebenablix ? "bg-emerald-50" : "bg-blue-50"}`}
          >
            ...
            <div className="space-y-2">
              {recs.slice(0, 5).map((rec, i) => (
                ...
                >
                  <span
                    className={`w-5 h-5 rounded-full flex items-center justify-center text-xs flex-shrink-0 text-white ${isWebenablix ? "bg-emerald-500" : "bg-[#2563EB]"}`}
                  >
                    {isWebenablix ? "✓" : i + 1}
                  </span>
                  {rec}
        ...
        <div className="p-6 space-y-4">
          ...
          <Section
            title={`Accessibility Issues (${accIssues.length})`}
            icon={Eye}
          >
            <div className="space-y-3">
              {accIssues.length === 0 && (
                <p className="text-emerald-600 text-center py-4 font-medium">
                  {isWebenablix
                    ? "✓ All accessibility issues resolved by Webenablix"
              ...
              {accIssues.length > 8 && (
                <p className="text-sm text-gray-500 text-center">
                  + {accIssues.length - 8} more issues
                </p>
              )}
            </div>
          </Section>
          ...
          <Section title={`SEO Issues (${seoIssues.length})`} icon={Globe}>
            <div className="space-y-3">
              {seoIssues.length === 0 && (
                <p className="text-emerald-600 text-center py-4 font-medium">
                  {isWebenablix
                    ? "✓ All SEO issues resolved by Webenablix"
                    : "No SEO issues found!"}
                </p>
              )}
              ...
              {seoIssues.length > 6 && (
                <p className="text-sm text-gray-500 text-center">
                  + {seoIssues.length - 6} more issues
                </p>
              )}
            </div>
          </Section>
          ...
          <Section
            title={`Resources & JavaScript (${resIssues.length})`}
            icon={Package}
          >
            <ResourcesSection
              issues={resIssues}
              summary={result.resources_summary}
            />
          </Section>

          <Section
            title={`Images & Media (${imgIssues.length})`}
            icon={ImageIcon}
          >
            <ImagesMediaSection
              issues={imgIssues}
              summary={result.images_summary}
            />
          </Section>
          ...
          <Section title={`Code Quality (${cqIssues.length})`} icon={Code2}>
            <CodeQualitySection
              issues={cqIssues}
              summary={result.code_quality_summary}
            />
          </Section>

          {fixes.length > 0 && (
            <Section
              title={`Suggested Fixes (${fixes.length})`}
              icon={Sparkles}
              defaultOpen={true}
            >
      ...
      {!isWebenablix && <LivePageInspector url={originalResult.url} />}
    </div>
  );
};
...
const TagChip = ({ tag, status, children }) => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded border text-xs font-mono font-semibold ${colors[status] || colors.neutral}`}
    >
      <span
        className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${dotColors[status] || dotColors.neutral}`}
      />
      &lt;{tag}&gt;
      {children && (
        <span className="font-sans font-normal ml-1 truncate max-w-[160px]">
          {children}
        </span>
      )}
    </span>
...
const DetailRow = ({ icon, label, status, detail, sub }) => {
  ...
  return (
    <div
      className={`flex items-start gap-3 p-3 rounded-lg border ${borderColor} ${bg}`}
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {sub && (
          <p className="text-xs text-gray-600 mt-0.5 break-words">{sub}</p>
        )}
      </div>
    </div>
  );
...
const HeadingGroup = ({ tag, data }) => {
  ...
  return (
    <div>
      <div
        className={`flex items-center gap-2 px-3 py-1.5 rounded-t-lg border-l-4 ${
          status === "good"
      ...
      >
        ...
        <span className="text-xs text-gray-500">
          {data.count} found{isH1 ? " — ideal: exactly 1" : ""}
        </span>
      </div>
      {data.items.length > 0 && (
...
const PageDOMAnalysis = ({ domAnalysis, security }) => {
  ...
  const titleOk = !!(
    d.page_title &&
    d.page_title_length >= 10 &&
    d.page_title_length <= 70
  );
  const titleWarn = !!(
    d.page_title &&
    (d.page_title_length < 10 || d.page_title_length > 70)
  );
  const descOk = !!(
    d.meta_description &&
    d.meta_description_length >= 50 &&
    d.meta_description_length <= 160
  );
  const descWarn = !!(
    d.meta_description &&
    (d.meta_description_length < 50 || d.meta_description_length > 160)
  );
    ...
    <div className="mt-10 max-w-5xl mx-auto">
      ...
      <div className="bg-white rounded-2xl shadow-2xl overflow-hidden">
          ...
          <div className="flex items-center gap-1.5">
            {security?.has_https ? (
              ...
              <>
                ...
                <span className="text-xs text-red-400">Not Secure</span>
              </>
            )}
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-2 divide-y lg:divide-y-0 lg:divide-x divide-gray-200">
          ...
          <div className="p-5 overflow-y-auto max-h-[900px] space-y-3">
            ...
            <DetailRow
              ...
              detail={
                d.meta_description
                  ? `${d.meta_description_length} chars`
                  : "Missing"
              }
              sub={
                d.meta_description
            ...
            <DetailRow
              ...
              status={
                d.og_title && d.og_description && d.og_image
                  ? "good"
                  : d.og_title
                    ? "warn"
                    : "error"
            ...
            <DetailRow
              ...
              sub={
                d.twitter_card
                  ...
                  : "Add twitter:card meta tag for rich Twitter/X share cards"
              }
            />
            ...
            <DetailRow
              ...
              sub={
                d.robots
                  ? `content="${d.robots}"`
                  : "No robots meta tag — search engines default to index and follow all links"
              }
            />
            ...
            <DetailRow
              ...
              detail={
                d.duplicate_ids.length === 0
                  ? "✓ None"
                  : `${d.duplicate_ids.length} duplicates`
              }
              sub={
                d.duplicate_ids.length > 0
                  ? `Duplicate IDs: ${d.duplicate_ids
                      .slice(0, 4)
                      .map((id) => `#${id}`)
                      .join(", ")} — breaks ARIA and JS selectors (WCAG 4.1.1)`
            ...
            <DetailRow
              icon={
                <StatusIcon
                  ok={imgBad === 0 && imgTotal > 0}
                  warn={imgBad > 0 && imgBad < imgTotal}
                />
              }
              label={`Images (${imgTotal} total)`}
              status={
                imgTotal === 0 ? "neutral" : imgBad > 0 ? "error" : "good"
              }
              detail={
                imgTotal === 0 ? "None found" : `${imgGood} ✓ / ${imgBad} ✗`
              }
              sub={
                imgTotal === 0
            ...
            <div className="mb-4">
              ...
              <div className="ml-4 space-y-1.5">
                <div className="flex items-center gap-2">
                  ...
                  >
                    {d.page_title
                      ? `"${d.page_title.substring(0, 40)}${d.page_title_length > 40 ? "…" : ""}"`
                      : "MISSING"}
                  </TagChip>
                  <span className="text-[10px] text-gray-400">
                    {d.page_title ? `${d.page_title_length}ch` : ""}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <TagChip
                ...
                <div className="flex items-center gap-2">
                  <TagChip tag="og:title" status={d.og_title ? "good" : "warn"}>
                    {d.og_title ? "present" : "missing"}
                  </TagChip>
                  <TagChip tag="og:image" status={d.og_image ? "good" : "warn"}>
                    {d.og_image ? "present" : "missing"}
                  </TagChip>
                </div>
                <div className="flex items-center gap-2">
                  <TagChip
              ...
              <div className="ml-4 space-y-4">
                  ...
                  return (
                    <div key={tag}>
                      {hd.items.slice(0, 3).map((text, i) => (
                        <div
                          key={i}
                          className={`flex items-start gap-2 mb-1 ${i > 0 ? "ml-4" : ""}`}
                        >
                          <TagChip tag={tag} status={status} />
                          <span
                            className={`${fontSizes[tag]} text-gray-700 break-words flex-1`}
                          >
                            {text || <em className="text-gray-400">empty</em>}
                          </span>
                        </div>
                      ...
                      {hd.count > 3 && (
                        <p className="text-[10px] text-gray-400 ml-6">
                          +{hd.count - 3} more &lt;{tag}&gt; tags
                        </p>
                      )}
                      {isH1 && hd.count === 0 && (
                        <div className="flex items-center gap-2">
                          <TagChip tag="h1" status="error">
                            MISSING
                          </TagChip>
                          <span className="text-xs text-red-600">
                            Required — use exactly one H1 per page
                          </span>
                        </div>
                      )}
                    </div>
                ...
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    {imgBad > 0 && (
                      <span className="text-[10px] bg-red-100 text-red-700 px-1.5 rounded-full font-semibold">
                        {imgBad} missing alt
                      </span>
                    )}
                    {d.images?.items?.filter((im) => im.isEmpty).length > 0 && (
                      <span className="text-[10px] bg-yellow-100 text-yellow-700 px-1.5 rounded-full font-semibold">
                        {d.images.items.filter((im) => im.isEmpty).length} empty
                        alt
                      </span>
                    )}
                    {imgGood > 0 && (
                      <span className="text-[10px] bg-emerald-100 text-emerald-700 px-1.5 rounded-full font-semibold">
                        {imgGood} with alt
                      </span>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {d.images?.items?.slice(0, 12).map((img, i) => {
                      ...
                      const borderColor = isMissingAlt
                        ? "border-red-400"
                        : isEmptyAlt
                          ? "border-yellow-400"
                          : "border-emerald-400";
                      const badgeBg = isMissingAlt
                        ? "bg-red-500"
                        : isEmptyAlt
                          ? "bg-yellow-500"
                          : "bg-emerald-500";
                      const badgeLabel = isMissingAlt
                        ? "✗ No alt"
                        : isEmptyAlt
                          ? "~ Empty alt"
                          : "✓ Has alt";
                      const BadgeIcon = isMissingAlt
                        ? XCircle
                        : isEmptyAlt
                          ? AlertTriangle
                          : CheckCircle;
                      const bottomBg = isMissingAlt
                        ? "bg-red-50"
                        : isEmptyAlt
                          ? "bg-yellow-50"
                          : "bg-emerald-50";
                      ...
                      return (
                        <div
                          key={i}
                          className={`relative rounded-lg border-2 overflow-hidden bg-gray-100 ${borderColor}`}
                        >
                          ...
                          <div
                            className={`absolute top-1.5 right-1.5 flex items-center gap-0.5 px-1.5 py-0.5 rounded-full text-white text-[9px] font-bold shadow ${badgeBg}`}
                          >
                          ...
                          {/* Info bar at bottom */}
                          <div
                            className={`px-2 py-1.5 text-[10px] leading-tight ${bottomBg}`}
                          >
                            {isMissingAlt && (
                              <p className="text-red-700 font-semibold">
                                alt attribute missing (WCAG 1.1.1)
                              </p>
                            )}
                            {isEmptyAlt && (
                              <p className="text-yellow-700 font-semibold">
                                alt="" — if decorative add role="presentation"
                              </p>
                            )}
                            {isGood && (
                              <p
                            ...
                            <p
                              className="text-gray-300 truncate mt-0.5 font-mono"
                              title={img.src}
                            >
                              {img.src
                                ? img.src
                  ...
                  {imgTotal > 12 && (
                    <p className="text-[10px] text-gray-400 mt-2 text-center">
                      Showing 12 of {imgTotal} images · {imgTotal - 12} more not
                      shown
                    </p>
                  )}
                </div>
                  ...
                  <div className="space-y-1">
                    {d.links?.items?.slice(0, 6).map((lnk, i) => (
                      <div key={i} className="flex items-start gap-2">
                        <TagChip
                          tag="a"
                          status={lnk.hasText ? "good" : "error"}
                        />
                        <span className="text-[10px] text-gray-600 break-all flex-1">
                          {lnk.hasText ? (
                            <span className="text-emerald-700">
                              "{lnk.text?.substring(0, 50)}"
                            </span>
                          ) : (
                            <span className="text-red-600 font-semibold">
                              no link text — inaccessible to screen readers
                            </span>
                          )}
                        </span>
                      </div>
                    ...
                    <div className="space-y-1">
                      {d.buttons?.items?.slice(0, 5).map((btn, i) => (
                        <div key={i} className="flex items-start gap-2">
                          <TagChip
                            tag="button"
                            status={btn.hasText ? "good" : "error"}
                          />
                          <span className="text-[10px] text-gray-600 break-words flex-1">
                            {btn.hasText ? (
                              <span className="text-emerald-700">
                                "{btn.text.substring(0, 50)}"
                              </span>
                            ) : (
                              <span className="text-red-600 font-semibold">
                                no accessible label (WCAG 4.1.2)
                              </span>
                            )}
                          </span>
                        </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-1">
                      {d.inputs?.items?.slice(0, 5).map((inp, i) => (
                        <div key={i} className="flex items-start gap-2">
                          <TagChip
                            tag={`input type="${inp.type}"`}
                            status={inp.hasLabel ? "good" : "error"}
                          />
                          <span className="text-[10px] text-gray-600 flex-1">
                            {inp.hasLabel ? (
                              ...
                              <span className="text-red-600 font-semibold">
                                no label — form field inaccessible (WCAG 1.3.1)
                              </span>
                            )}
                          </span>
                        </div>
        ...
        <div className="px-6 py-3 bg-gray-50 border-t flex flex-wrap gap-x-6 gap-y-1">
          {[
            ["good", "bg-emerald-500", "Pass / Good"],
            ["warn", "bg-yellow-500", "Warning / Improvable"],
            ["error", "bg-red-500", "Error / Fail"],
            ["neutral", "bg-gray-400", "Informational"],
          ].map(([s, c, l]) => (
            <div
              key={s}
...
const LivePageInspector = ({ url }) => {
  ...
  useEffect(() => {
    ...
    const measure = () => {
      if (ctrRef.current) setCtrW(ctrRef.current.offsetWidth - 2);
    };
    measure();
      ...
      <div className="mt-10 max-w-5xl mx-auto">
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-8 text-center">
          ...
          <h3 className="text-white font-semibold text-lg mb-2">
            Inspect Actual Page with Annotations
          </h3>
          <p className="text-white/60 text-sm mb-6 max-w-lg mx-auto">
            Load the real website inside an inspector and see every
            accessibility error, warning, and good practice marked directly on
            the actual page elements — headings, images, links, buttons, and
            form inputs.
          </p>
          <button
    ...
    <div className="mt-10 max-w-5xl mx-auto">
      ...
      <div className="bg-white rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-slate-800 to-slate-700 px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1 bg-slate-900/60 rounded px-3 py-1.5 text-xs text-slate-300 font-mono truncate">
            {url}
          </div>
          <button
            onClick={startInspect}
        ...
        {phase === "ready" && (
          <div className="px-4 py-2 bg-gray-50 border-b flex items-center gap-2 flex-wrap">
            ...
            {[
              ["all", "All", null],
              ["error", "Errors", "bg-red-500"],
              ["warn", "Warnings", "bg-yellow-500"],
              ["good", "Good", "bg-emerald-500"],
              ...
              >
                ...
                {val !== "all" && counts[val] > 0 && (
                  <span className="opacity-70">({counts[val]})</span>
                )}
                {val === "all" && (
                  <span className="opacity-70">({annotations.length})</span>
                )}
              </button>
            ))}
          </div>
        ...
        <div className="flex" style={{ minHeight: 500 }}>
            ...
            {phase === "blocked" && (
              <div className="absolute inset-0 bg-white/95 flex flex-col items-center justify-center z-50 gap-3 p-8">
                ...
                <p className="text-sm text-gray-500 text-center max-w-sm">
                  This site uses security policies that prevent it from loading
                  inside an iframe. The element data in the Deep Analysis
                  section above is still accurate.
                </p>
              </div>
            )}
            ...
            {phase !== "idle" && (
              ...
              >
                ...
                >
                  {filtered.map((ann) => {
                    const c =
                      ANNOTATION_COLORS[ann.status] || ANNOTATION_COLORS.info;
                      ...
                      >
                        ...
                        {isH && (
                          ...
                          >
                            ...
                            >
                              {ann.label} · {ann.status.toUpperCase()}
                            </div>
                            <div
                              style={{
                                marginBottom: ann.issue ? 4 : 0,
          ...
          {phase === "ready" && (
            <div
              className="w-72 flex-shrink-0 border-l border-gray-200 bg-slate-50 overflow-y-auto"
              style={{ maxHeight: 680 }}
            >
              ...
              <div className="p-3 border-t bg-white mt-auto">
                ...
                {[
                  ["error", "bg-red-500", "Error — must fix"],
                  ["warn", "bg-yellow-500", "Warning — should fix"],
                  ["good", "bg-emerald-500", "Good practice"],
                  ["info", "bg-blue-500", "Informational"],
                ].map(([s, c, l]) => (
                  <div
                    key={s}
                  ...
                  >
                    <span
                      className={`w-2 h-2 rounded-full flex-shrink-0 ${c}`}
                    />
                    {l}
                  </div>
                ))}
...
const AuditSection = ({ externalUrl }) => {
  ...
  const displayResult =
    auditResult && activeView === "webenablix"
  ...
  return (
    <section
      id="audit-section"
      className="py-20 lg:py-28 bg-gradient-to-br from-[#1e3a5f] to-[#0f172a] relative overflow-hidden"
    >
      ...
      <div className="relative max-w-7xl mx-auto px-4">
        <div className="text-center">
          ...
          <p className="text-white/70 text-lg mb-8 max-w-2xl mx-auto">
            Analyze accessibility, SEO, Core Web Vitals, mobile-friendliness,
            and security
          </p>
