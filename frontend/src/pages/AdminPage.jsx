...
const PlanBadge = ({ plan }) => {
  ...
  return (
    <span
      className={`px-2 py-0.5 rounded-full text-xs font-semibold capitalize ${map[plan] || map.free}`}
    >
      {plan || "free"}
    </span>
  );
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        ...
        <form
          ...
          className="space-y-4 bg-white shadow-xl rounded-xl p-6 border border-gray-100"
        >
          <div>
...
const AdminPage = () => {
  ...
  const [isAuthenticated, setIsAuthenticated] = useState(
    !!localStorage.getItem("webenablix_admin_token"),
  );
  ...
  const handleSaveBlog = async () => {
    ...
    try {
      ...
      const url = isEdit
        ? `${API_URL}/api/admin/blogs/${blogForm.id}`
        : `${API_URL}/api/admin/blogs`;
      const method = isEdit ? "PUT" : "POST";
      const payload = {
        ...blogForm,
        feature_image_url:
          blogForm.feature_image_url || blogForm.image_url || "",
        image_url: blogForm.feature_image_url || blogForm.image_url || "",
        additional_images: Array.isArray(blogForm.additional_images)
          ? blogForm.additional_images
          : [],
      };
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-gray-100 flex">
      ...
      {toast && (
        <div
          className={`fixed top-4 right-4 z-50 px-4 py-3 rounded-lg shadow-xl text-sm font-medium flex items-center gap-2 ${
            toast.type === "error"
        ...
        <header className="bg-gray-900 border-b border-gray-800 px-8 py-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-gray-400">
              Welcome to the Webenablix Admin Panel
            </p>
          </div>
          <button
            onClick={fetchStats}
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <StatCard
                      icon={Globe}
                      label="Total Audits"
                      value={stats.audits.total_audits}
                      sub={`+${stats.audits.audits_this_week} this week`}
                      color="green"
                    />
                    <StatCard
                      icon={TrendingUp}
                      label="Avg Score"
                      value={`${stats.audits.avg_score}%`}
                    ...
                    <StatCard
                      icon={AlertTriangle}
                      label="Failed Scans"
                      value={stats.audits.failed_audits}
                      sub="scan errors"
                      color="red"
                    />
                  </div>
                    ...
                    <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
                      ...
                      <div className="space-y-3">
                        {[
                          {
                            label: "Free",
                            val: stats.users.free_users,
                            color: "bg-gray-600",
                          },
                          {
                            label: "Paid",
                            val: stats.users.paid_users,
                            color: "bg-blue-500",
                          },
                        ].map(({ label, val, color }) => {
                          const pct =
                            stats.users.total_users > 0
                              ? Math.round(
                                  (val / stats.users.total_users) * 100,
                                )
                              : 0;
                          return (
                            <div key={label}>
                      ...
                      {stats.daily_audits.length === 0 ? (
                        ...
                        <div className="flex items-end gap-1 h-24">
                          {stats.daily_audits.map((d) => {
                            const max = Math.max(
                              ...stats.daily_audits.map((x) => x.count),
                            );
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div className="text-xs text-gray-400">
                          {stats.users.total_users} registered users
                        </div>
                      </div>
                    </button>
                    <button
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div className="text-xs text-gray-400">
                          {stats.audits.total_audits} scans performed
                        </div>
                      </div>
                    </button>
                  </div>
              ...
              <div className="flex items-center gap-3">
                <div className="relative flex-1 max-w-sm">
                  ...
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter") fetchUsers(1, userSearch);
                    }}
            ...
            <div className="space-y-4">
                      ...
                      {audits.map((a) => (
                        ...
                        >
                          ...
                          <td className="px-5 py-3 text-center">
                            <ScorePill score={a.overall_score || 0} />
                          </td>
                          <td className="px-5 py-3 text-center">
                            <ScorePill score={a.accessibility_score || 0} />
                          </td>
                          <td className="px-5 py-3 text-center">
                            <ScorePill score={a.seo_score || 0} />
                          </td>
                          <td className="px-5 py-3 text-center">
                            <ScorePill score={a.resources_score || 0} />
                          </td>
                          <td className="px-5 py-3 text-center">
                            <ScorePill score={a.images_score || 0} />
                          </td>
                          <td className="px-5 py-3 text-center">
                            <ScorePill score={a.network_caching_score || 0} />
                          </td>
                          <td className="px-5 py-3 text-center">
                            <ScorePill score={a.code_quality_score || 0} />
                          </td>
              ...
              {auditsPages > 1 && (
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm text-gray-400">
                    Page {auditsPage} of {auditsPages}
                  </span>
                  <button
                    onClick={() => fetchAudits(auditsPage + 1)}
                    ...
                    <tbody className="divide-y divide-gray-800">
                      {blogs.map((b) => (
                        ...
                        >
                          <td className="px-5 py-3">
                            <div
                              className="font-medium text-gray-100 max-w-[220px] truncate"
                              title={b.title}
                            >
                              {b.title}
                            </div>
                          ...
                          <td className="px-5 py-3">
                            {b.category ? (
                              <span className="px-2 py-0.5 rounded-full text-xs font-semibold bg-blue-900/50 text-blue-300">
                                {b.category}
                              </span>
                            ) : (
                            ...
                            <button
                              onClick={() =>
                                setBlogForm({
                                  ...
                                  slug: b.slug || "",
                                  feature_image_url:
                                    b.feature_image_url || b.image_url || "",
                                  image_url:
                                    b.image_url || b.feature_image_url || "",
          ...
          <div className="bg-gray-900 border border-gray-700 rounded-xl w-full max-w-2xl shadow-2xl">
            <div className="flex items-center justify-between p-6 border-b border-gray-800">
              <h3 className="font-bold text-lg">
                {blogForm.id ? "Edit Blog Post" : "New Blog Post"}
              </h3>
              <button
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setBlogForm((f) => ({ ...f, excerpt: e.target.value }))
                  }
                  rows={2}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setBlogForm((f) => ({ ...f, category: e.target.value }))
                    }
                    className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-100 focus:outline-none focus:border-blue-500"
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-100 focus:outline-none focus:border-blue-500"
                  >
                    {["blue", "green", "purple", "orange", "red", "gray"].map(
                      (c) => (
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setBlogForm((f) => ({ ...f, author: e.target.value }))
                    }
                    className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-100 focus:outline-none focus:border-blue-500"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    value={blogForm.date}
                    onChange={(e) =>
                      setBlogForm((f) => ({ ...f, date: e.target.value }))
                    }
                    className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-100 focus:outline-none focus:border-blue-500"
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setBlogForm((f) => ({ ...f, read_time: e.target.value }))
                    }
                    className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-100 focus:outline-none focus:border-blue-500"
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setBlogForm((f) => ({ ...f, meta_title: e.target.value }))
                  }
                  className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-100 focus:outline-none focus:border-blue-500"
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                <span className="text-sm text-gray-300 flex items-center gap-1.5">
                  <Star className="h-4 w-4 text-yellow-400" /> Mark as featured
                  post
                </span>
              </label>
            </div>
